        ...
        if (!user) {
          ...
          Users.create(userData, function (err, user) {
            if (err) {
              ...
              user = user.cloneUser()
              return resolve(responseStatus.Code200({ user: user, token: token, message: responseStatus.SIGN_UP_SUCCESS }))
            }
          })
        } else {
          return reject(responseStatus.Code409({ errorMessage: responseStatus.EXIST_EMAIL }))
        }
      })
    } else return reject(responseStatus.Code400())
  })
}
